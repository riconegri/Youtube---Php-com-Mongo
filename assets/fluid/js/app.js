var timer;
/**
 * Elemento do campo de busca
**/
var twitter_button = $('#twitter_button');
var question = $('#searchinput');
var buscas = $('#termos_search');
var busca_atual = $('#videos_buscas_atual');
var data_youtube = false;
var i =0;
var pi = 0;
...
function listVideos(json,divid) {
    ...
    if(json.data.items){
        for (var i = 0; i < json.data.items.length; i++) {
            ...
            //alert(li_html(entry));break;
            appendOptionLast(li_html(entry),entry.id,'ul1');
        }
    }else{
...
function insertVideos(div,typ,q,results,start){
    ...
    if(typ == "search"){
        script.setAttribute('src', 'http://gdata.youtube.com/feeds/api/videos?q='+q
            +'&v=2&start-index='+start
            +'&max-results='+results
            +'&format=5&alt=jsonc&callback=youtubeInit['+l+']');
        if(document.title)
            document.title = "Busca: "+q.replace("+"," ")+" - YouTube-Twitter";
    }
    
    if(typ == "hot"){
        script.setAttribute('src', 'http://gdata.youtube.com/feeds/api/standardfeeds/recently_featured?alt=jsonc&v=2&format=5&callback=youtubeInit['+l+']&start-index=1&max-results=32');
        if(document.title)
            document.title = "Recentemente destacados - YouTube Fast Search";
    }
    if(typ == "mostviewed"){
        ...
        if(document.title)
            document.title = "Mais Visualizados este mês - YouTube Fast Search";
    }
    if(typ == "linked"){
        ...
        if(document.title)
            document.title = "Mais linkados este mês - YouTube Fast Search";
    }
    if(typ == "user"){
...
function updateNormalPlayerInfo() {
    ...
    if((time > dur) && dur> 1 && time > 1){
        ...
        getNextPlaylist();
        if(document.title)
            document.title = "Próximo na lista...";
    }
}
//var slideit;
...
function appendOptionLast(text,id,ul){
    try{
        ...
        alert('Problema ao carregar vídeos');
    }
}
...
function userSearch(){
    clearList('ul1');
    var user = encodeURI(question.val());
    insertVideos('ul1','user',user,'20','0');
}
function mostViewed(){
    clearList('ul1');
...
function makeRequest(type,word){
    clearList('ul1');
    var tags = !word ? encodeURI(question.val()) : word;
    if (word){
        question.val(word);
    }else{
        var html_append = busca_html(type);
        $(html_append).appendTo(buscas);
    }
    var remote = new rpc(type,tags);
    remote.ajx();
    busca_atual.text(question.val());
    
    insertVideos('ul1','search',tags,'40','0');
}
...
function playVideo(id,loader,title,clearer){
    ...
    dt_video = getDataById(id);
    appendOptionLast(li_html(dt_video),id,'ul4');
    var remote = new rpc('visualizados',id);
    remote.title = title;
    remote.ajx();
    if(document.title)
...
function login(assertion,but,mess) {
    $.ajax({
        type: "POST",
        url: Path.app + "bid",
        data: {
        ...
        if(resposta.status == 'okay'){
            message = 'Bem vindo ' + resposta.email;
            but.parent().empty();
            mess.text(message);
        }else{
            alert( 'Problema ao logar, tente novamente' );
        }
    }).fail(function(jqXHR, textStatus) {
        alert( 'Problema ao logar, tente novamente: ' + textStatus );
    });
}

//METODOS DE JSON DO YOUTUBE
function getDataById(id){
    for(i=0;i<data_youtube.length;i++){
        if(data_youtube[i].id == id){
            return data_youtube[i];
        }
    }
    return false;
}
...
//TEMPLATES
function busca_html(tipo){
    var tipos = {
        'search': '\n<a href="javascript:makeRequest(\'search\',\''
        +decodeURI(question.val())+'\')" class="label label-success">'+decodeURI(question.val())+'</a>\n'
    };
    return tipos[tipo];
    
}

function li_html (json_data){
    return '<a href="javascript:playVideo(\''
    +json_data.id+'\',false,\''+addslashes(json_data.title)+'\',true)" title="'
    +json_data.title+'"><img src="'+
    json_data.thumbnail.sqDefault
    +'" onmouseout="mouseOutImage(this)" onmouseover="mousOverImage(this,\''+json_data.id+'\',1)"></a><br />';
}

//METODOS TWITTER
function tb(){
    twitter_button.empty();
    if(twitter_button.html(twitter_button_html())){
        var tweet_button = new twttr.TweetButton( twitter_button.get(0) );
        tweet_button.render();
    }
}

function twitter_button_html(){
    return '<a href="http://twitter.com/share" data-url="http://www.youtube.com/watch?v='+dt_video.id
    +'&feature=g-all-pls" data-text="Que vídeo maneiro! ' + dt_video.title
